...
#ifndef DoCompute_numRepstde_HH_
#define DoCompute_numRepstde_HH_

#include <systemc>
using namespace std;
SC_MODULE(DoCompute_numRepstde) {
    static const unsigned int DATA_WIDTH = 32;
    static const unsigned int ADDR_WIDTH = 1;
    static const unsigned int DoCompute_numRepstde_depth = 2;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[DoCompute_numRepstde_depth];
    ...
    SC_CTOR(DoCompute_numRepstde) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < DoCompute_numRepstde_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (DoCompute_numRepstde_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < DoCompute_numRepstde_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (DoCompute_numRepstde_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > DoCompute_numRepstde_depth) {
...
#endif //DoCompute_numRepstde_HH_
